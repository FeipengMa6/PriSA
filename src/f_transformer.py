    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None,mask=None):
        ...
        if x_k is None and x_v is None:
            x, attn_map = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True) 
            x, attn_map = self.self_attn(query=x, key=x_k, value=x_v, attn_mask=mask)
...
def padding_mask(B,seq_len,content_len,pad_value=0):
    mask = torch.ones(B,seq_len)
    for i,l in enumerate(content_len):
        mask[i,1:content_len[i]+1] = 0
    mask = mask.masked_fill(mask==1.,-float('inf'))
    mask[:,0]=1.0
    return mask.cuda()
def text_padding_mask(mask):
    mask = mask.float()
